...
(function (root) {
  ...
  class Program {
    ...
    measured(n, physical) { return this.scan > 1 && n in this.t ? this.t[n] : physical; }   // what the logic sees: last published (possibly faulted) value
  ...
  class Intake extends Program {
    ...
    logic(dt) { const lahhL = this.clamp('LAHH_LIMIT', 60, 99), maxS = this.clamp('PUMP_MAX_SPEED', 20, 100), stopSp = this.clamp('LEVEL_STOP_SP', 20, lahhL - 2), startSp = this.clamp('LEVEL_START_SP', 5, stopSp - 5), speedSp = this.clamp('PUMP_SPEED_SP', 0, maxS); this.clamp('LALL_LIMIT', 2, startSp - 2); this.clamp('TURB_HIGH_LIMIT', 1, 200);
      const level = this.measured('LT-101', this.level), turbidity = this.measured('AT-101', this.turbidity);
      if (this.resetRequested() && level < lahhL - 2) this.lahh = false; if (level >= lahhL) this.lahh = true;
      const auto = this.coil('AUTO_MODE');
      if (auto) { this.consume('PUMP_START_CMD'); this.consume('PUMP_STOP_CMD'); if (level < startSp) this.pumpRun = true; else if (level > stopSp) this.pumpRun = false; this.valveOpen = this.pumpRun; }
      ...
      this.interlockWord = this.lahh ? 1 : 0; this.alarmWord = (turbidity > this.get('TURB_HIGH_LIMIT') ? 2 : 0) | (level < this.get('LALL_LIMIT') ? 4 : 0); }
  ...
  class Treatment extends Program {
    ...
    sequence(dt, start, stop, available, flow) { const flowSp = this.get('FLOW_SP'); if (stop && this.seq !== 0 && this.seq !== 4) { this.seq = 4; this.seqTimer = 0; } else if (start && this.seq === 0) { this.seq = 1; this.seqTimer = 0; }
      if (this.seq === 1) { if (available && this.upLevel > 15) { this.seq = 2; this.seqTimer = 0; } } else if (this.seq === 2) { this.seqTimer += dt; if (flow > 0.8 * flowSp) { this.seq = 3; this.seqTimer = 0; } else if (this.seqTimer > 1) this.seq = 5; } else if (this.seq === 3) { if (!available) { this.seq = 5; this.seqTimer = 0; } } else if (this.seq === 5) { if (available && this.upLevel > 15) { this.seq = 2; this.seqTimer = 0; } } else if (this.seq === 4) { this.seqTimer += dt; if (flow < 1 || this.seqTimer > 0.2) this.seq = 0; } }
    logic(dt) { const aahh = this.clamp('AAHH_LIMIT', 2, 8), cl2Sp = this.clamp('CL2_SP', 0.2, aahh - 0.5), flowSp = this.clamp('FLOW_SP', 20, 200), bwSp = this.clamp('BACKWASH_DP_SP', 20, 100), maxStroke = this.clamp('MAX_STROKE', 10, 100), manStroke = this.clamp('MANUAL_STROKE_SP', 0, maxStroke); this.clamp('AALL_LIMIT', 0.1, cl2Sp); const kp = this.clamp('PI_KP', 0.1, 20), ti = this.clamp('PI_TI', 5, 600);
      const residual = this.measured('AT-201', this.residual), dp = this.measured('PDT-201', this.dp), flow = this.measured('FT-201', this.flow);
      if (this.resetRequested() && residual < aahh - 0.5) this.aahh = false; if (residual >= aahh) this.aahh = true;
      ...
      const auto = this.coil('AUTO_MODE'); if (this.consume('BACKWASH_CMD') && this.bwLeft <= 0) this.bwLeft = 0.1; if (dp >= bwSp && this.bwLeft <= 0) this.bwLeft = 0.1; const bw = this.bwLeft > 0;
      const start = this.consume('PLANT_START_CMD'), stop = this.consume('PLANT_STOP_CMD');
      if (auto) { this.upLevel = upLevel; this.sequence(dt, start, stop, available, flow); this.transferRun = (this.seq === 2 || this.seq === 3) && available && !bw && !this.cwHold; } else this.transferRun = this.coil('TRANSFER_RUN_CMD') && available && !bw;
      this.flowCmd = this.transferRun ? flowSp : 0; const wanted = flow > 5 && (!auto || this.seq === 3);
      if (auto) { this.dosingRun = wanted && !this.aahh; if (this.dosingRun) { const dts = dt * 3600, err = cl2Sp - residual; this.stroke += (kp * (err - this.prevErr) + kp / ti * err * dts) * 10; this.prevErr = err; this.stroke = Math.max(0, Math.min(maxStroke, this.stroke)); } }
      ...
      this.interlockWord = this.aahh ? 1 : 0; const low = this.dosingRun && residual < this.get('AALL_LIMIT'); this.alarmWord = (low ? 2 : 0) | (dp > bwSp * 0.9 ? 4 : 0); }
  ...
  class Distribution extends Program {
    ...
    logic(dt) { const pahh = this.clamp('PAHH_LIMIT', 3, 8), sp = this.clamp('PRESSURE_SP', 2, pahh - 1), delta = this.clamp('LAG_START_DELTA', 0.1, 2), lall = this.clamp('LALL_LIMIT', 2, 40), maxS = this.clamp('MAX_SPEED', 30, 100); this.clamp('PALL_LIMIT', 0.5, sp - 0.5);
      const level = this.measured('LT-301', this.level), pressure = this.measured('PT-301', this.pressure);
      if (this.resetRequested()) { if (level > lall + 5) this.lall = false; if (pressure < pahh - 0.5) this.pahh = false; } if (level <= lall) this.lall = true; if (pressure >= pahh) this.pahh = true; const tripped = this.lall || this.pahh;
      const auto = this.coil('AUTO_MODE');
      if (auto) { this.consume('STOP_ALL_CMD'); this.run[0] = !tripped; const err = sp - pressure, dts = dt * 3600; this.integral = Math.max(-50, Math.min(50, this.integral + err * dts * 0.05)); this.speedCmd = Math.max(0, Math.min(maxS, 70 + err * 25 + this.integral));
        if (pressure < sp - delta && this.run[0]) this.lagTimer++; else if (pressure >= sp - delta / 4 && this.demand < this.PUMP * 0.8) this.lagTimer--; this.lagTimer = Math.max(-40, Math.min(10, this.lagTimer)); if (this.lagTimer >= 10) this.run[1] = true; if (this.lagTimer <= -40) this.run[1] = false; this.run[1] = this.run[1] && !tripped; }
      ...
      this.interlockWord = (this.lall ? 1 : 0) | (this.pahh ? 2 : 0); this.alarmWord = (pressure < this.get('PALL_LIMIT') && (this.run[0] || this.run[1])) ? 4 : 0; }
